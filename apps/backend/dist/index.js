...
app.get("/api/machines", async (req, res) => {
    ...
    if (search) {
        const result = await pool.query("SELECT * FROM machines WHERE name ILIKE $1 ORDER BY name", [`%${search}%`]);
...
app.get("/api/machines/:id", async (req, res) => {
    ...
    const activity = await pool.query("SELECT id, machine_id, user_id, status, tests_count, test_ids, started_at, finished_at, estimated_duration FROM test_runs WHERE machine_id=$1 AND status='running' ORDER BY started_at DESC LIMIT 1", [id]);
...
app.post("/api/machines/:id/reservations", requireAuth, async (req, res) => {
    ...
    try {
        ...
        const machineRes = await client.query("SELECT id, status FROM machines WHERE id=$1 FOR UPDATE", [machineId]);
...
app.post("/api/machines/:id/lock", requireAuth, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        if (force) {
            const cancelled = await client.query("UPDATE reservations SET status='cancelled' WHERE machine_id=$1 AND status='active' RETURNING user_id, session_name", [machineId]);
...
app.get("/api/reservations", requireAuth, async (req, res) => {
    ...
    const result = await pool.query(`SELECT r.*, m.name AS machine_name FROM reservations r
     JOIN machines m ON m.id=r.machine_id
     WHERE ${where}
     ORDER BY r.created_at DESC`, values);
    res.json({ reservations: result.rows });
});
app.get("/api/all-reservations", requireAuth, async (_req, res) => {
    const result = await pool.query(`SELECT r.*, m.name AS machine_name FROM reservations r
     JOIN machines m ON m.id=r.machine_id
...
app.post("/api/reservations/:id/complete", requireAuth, async (req, res) => {
    ...
    try {
        await client.query("BEGIN");
        const resv = await client.query("SELECT * FROM reservations WHERE id=$1 AND user_id=$2 FOR UPDATE", [reservationId, authReq.userId]);
        if (!resv.rowCount) {
            await client.query("ROLLBACK");
...
app.get("/api/notifications", requireAuth, async (req, res) => {
    const authReq = req;
    const result = await pool.query("SELECT * FROM notifications WHERE user_id=$1 ORDER BY created_at DESC", [authReq.userId]);
...
app.post("/api/tests/run", requireAuth, async (req, res) => {
    ...
    try {
        ...
        const runRes = await client.query("INSERT INTO test_runs (machine_id, user_id, status, tests_count, test_ids, estimated_duration) VALUES ($1,$2,'running',$3,$4,$5) RETURNING *", [machineId, authReq.userId, testIds.length, testIds, estimatedDuration]);
        await client.query("COMMIT");
...
app.get("/api/test-runs", async (_req, res) => {
    await reconcileMachineStatuses();
    try {
        const result = await pool.query(`SELECT * FROM test_runs 
