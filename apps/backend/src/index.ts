...
app.get("/api/machines/:id", async (req: express.Request, res: express.Response) => {
  ...
  const activity = await pool.query(
    "SELECT id, machine_id, user_id, status, tests_count, started_at, finished_at, estimated_duration FROM test_runs WHERE machine_id=$1 AND status='running' ORDER BY started_at DESC LIMIT 1",
    [id]
  );
  ...
  try {
    ...
    if (machineRes.rows[0].status === "reserved") {
      ...
      await createNotification(
        authReq.userId,
        "Reservation warning",
        "Reservation cannot be created because the machine is already reserved.",
        "warning"
      );
  ...
  try {
    ...
    if (machineRes.rows[0].status === "reserved" && !force) {
      ...
      await createNotification(
        authReq.userId,
        "Lock warning",
        "Machine lock is blocked because the machine is reserved.",
        "warning"
      );
      return res.status(409).json({ error: "Machine is reserved" });
    }
    if (machineRes.rows[0].status === "busy" && !force) {
      ...
      await createNotification(
        authReq.userId,
        "Lock warning",
        "Machine lock is blocked because tests are currently running.",
        "warning"
      );
  ...
  try {
    ...
    if (machineRes.rows[0].status !== "locked" && !adminRes.rowCount) {
      ...
      await createNotification(
        authReq.userId,
        "Unlock warning",
        "Machine is not locked, so unlock was skipped.",
        "warning"
      );
...
app.post("/api/reservations/:id/complete", requireAuth, async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    await createNotification(
      authReq.userId,
      "Job completed",
      `Session '${reservation.session_name}' finished on ${reservation.end_at}`,
      "success"
    );
  ...
  try {
    ...
    if (machineRes.rows[0].status !== "available") {
      ...
      await createNotification(
        authReq.userId,
        "Run warning",
        "Tests cannot start because the selected machine is not available.",
        "warning"
      );
    ...
    const runRes = await client.query(
      "INSERT INTO test_runs (machine_id, user_id, status, tests_count, estimated_duration) VALUES ($1,$2,'running',$3,$4) RETURNING *",
      [machineId, authReq.userId, testIds.length, estimatedDuration]
    );
    const testRunId = runRes.rows[0].id;
    for (const testId of testIds) {
      await client.query(
        "INSERT INTO test_run_items (test_run_id, test_id) VALUES ($1,$2)",
        [testRunId, testId]
      );
    }
    await client.query("COMMIT");
